...
def _generate_with_rotation(history: list, base_parts: list, tmp_path: str | None):
...
async def chat(
    ...
    try:
        ...
        response, _sent_parts = _generate_with_rotation(history, [message], tmp_path)
        ...
        history.append(types.Content(role="user", parts=[message]))
        history.append(types.Content(role="model", parts=[response.text]))
